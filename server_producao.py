...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_atualizar(self):
        ...
        print(f"DEBUG raw body size: {len(raw)} bytes")
        print(f"DEBUG raw body start: {raw[:200]}")
        ...
        if not csv_data:
            self._json({'error':f'Arquivo CSV não enviado. files={list(files.keys())} fields={list(fields.keys())} ct={ct} len={length} body_size={len(raw)}'},400); return
