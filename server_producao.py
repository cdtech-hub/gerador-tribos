"""
server_producao.py — Sistema Guardiões do Amor Maior
=====================================================
Versão completa com todas as funcionalidades:
  - Login por senha
  - Upload de CSV → atualiza planilha preservando FCs/tribos
  - Gestão de conflitos/restrições
  - Geração de etiquetas ZPL (todos ou só novatos)
...
Configurações:
    SENHA  — senha de acesso (troque antes de subir!)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_atualizar(self):
        ...
        raw = self.rfile.read(int(length)) if length else self.rfile.read(10 * 1024 * 1024)
        files, fields = parse_multipart(ct, raw)
        # Busca o arquivo: primeiro tenta 'csv', depois pega o primeiro arquivo disponível
        csv_data = files.get('csv') or (list(files.values())[0] if files else None)
        if not csv_data:
            self._json({'error':'Arquivo não enviado.'},400); return
        # Tenta pegar o nome do arquivo para determinar extensão
        if not fields.get('filename') and files:
            fields['filename'] = list(files.keys())[0]
    ...
    def _handle_fichas(self):
        if not os.path.exists(MODELO):
            self._json({'error': f'Modelo nao encontrado: {MODELO}'}, 500); return
        try:
            import openpyxl
            from gerar_fichas import gerar_fichas_pdf
            ...
            def g(row, col):
                ...
                if not idx: return ''
                v = row[idx-1]
                return str(v).strip() if v is not None and str(v).strip().lower() not in ['nan','none'] else ''
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):
                ...
                campistas.append({
                    'fc': fc, 'nome': nome, 'tribo': g(row,'TRIBO'), 'sexo': g(row,'SEXO'),
                    'nasc': g(row,'DATA DE NASCIMENTO'), 'celular': g(row,'CELULAR'),
                    'email': g(row,'E-MAIL'), 'cidade': g(row,'CIDADE'),
                    'paroquia': g(row,'PAROQUIA'), 'profissao': g(row,'PROFISSAO'),
                    'camiseta': g(row,'CAMISETA'), 'peso': g(row,'PESO'), 'altura': g(row,'ALTURA'),
                    'est_civil': g(row,'ESTADO CIVIL'), 'doc_tipo': 'CPF/RG', 'doc_num': g(row,'DOCUMENTO'),
                    'c1nome': g(row,'CONTATO 1'), 'c1tel': g(row,'TEL CONTATO 1'),
                    'c2nome': g(row,'CONTATO 2'), 'c2tel': g(row,'TEL CONTATO 2'),
                    'alergias': g(row,'ALERGIAS'),
                })
            ...
            fcs_filter = params.get('fcs')
            if fcs_filter:
                campistas = [c for c in campistas if c['fc'] in fcs_filter]
            if not campistas:
                self._json({'error': 'Nenhum campista encontrado.'}, 400); return

            pdf_bytes = gerar_fichas_pdf(campistas)
            pdf_b64 = base64.b64encode(pdf_bytes).decode()
            self._json({'ok': True, 'pdf_b64': pdf_b64, 'n_fichas': len(campistas)})
        except Exception as e:
            traceback.print_exc()
            self._json({'error': str(e)}, 500)
